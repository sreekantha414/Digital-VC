...
class _CreateCardScreenState extends State<CreateCardScreen> {
  ...
  // Dynamic fields
  final _dynamicKeyController = TextEditingController();
  final Map<String, TextEditingController> _dynamicFields = {};

  // Predefined keys list (to avoid duplication in dynamic)
  final List<String> _predefinedKeys = [
    "name",
    "jobTitle",
    "company",
    "phone",
  ...
  void initState() {
    ...
    if (widget.existingCard != null) {
      ...
      // Load unknown fields into dynamic fields
      fields.forEach((key, value) {
        if (!_predefinedKeys.contains(key)) {
          _dynamicFields[key] = TextEditingController(text: value ?? '');
        }
      });
    }
  }
  ...
  void _addDynamicField() {
    final key = _dynamicKeyController.text.trim();
    if (key.isEmpty) {
      AlertUtils.showSnackbarWarning(context, 'Please enter a key name');
      return;
    }
    if (_dynamicFields.containsKey(key) || _predefinedKeys.contains(key)) {
      AlertUtils.showSnackbarWarning(context, 'Key "$key" already exists');
      return;
    }
    setState(() {
      _dynamicFields[key] = TextEditingController();
    });
    _dynamicKeyController.clear();
  }
  ...
  void dispose() {
    ...
    _dynamicKeyController.dispose();
    _dynamicFields.values.forEach((controller) => controller.dispose());
    super.dispose();
  }
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if (_dynamicFields.isNotEmpty) const Text("Other Fields", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),

                    // Show dynamic fields
                    for (var entry in _dynamicFields.entries)
                      CustomTextField(label: entry.key, hint: "Enter ${entry.key}", controller: entry.value),

                    const SizedBox(height: 20),
                    const Text("Add Custom Field", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    Row(
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              hintText: "Enter key name (e.g., dob)",
